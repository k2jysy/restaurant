...
function shortcode_contactform($atts, $content = null) {
   extract(shortcode_atts(array(
	...
	$message_body  = "You have been contacted by $name with regards to $mail_subject.\r\n\n";
	$message_body .= "You can contact $name via email, $email.\r\n\n";

	if(mail($email, $mail_subject, $message_body, $headers))
		$output .= $success_msg;
	 else
		$output .= $error_msg;			
	}
	...
	$output .= '<div class="row100">';
	$output .= '<div class="col70"><input id="c_email" name="c_email" class="digits" placeholder="Telephone"/><i class="fa fa-phone"></i></div>';
	$output .=	'</div>';
	} 
	
	if(!empty($display_address) && $display_address == 'yes') { 
	$output .= '<div class="row100">';
	$output .= '<div class="col70"><input id="c_email" name="c_email" placeholder="Address"/><i class="fa fa-map-marker"></i></div>';
	$output .=	'</div>';
	} 
	
	if(!empty($display_city) && $display_city == 'yes') { 
	$output .= '<div class="row100">';
	$output .= '<div class="col70"><input id="c_email" name="c_email" placeholder="City"/><i class="fa fa-map-marker"></i></div>';
	$output .=	'</div>';
	} 	
				...
				$output .= '<div class="image image-block">';
					...
					$output .= '<div class="block_hover">';
						...
						$output .= '<div class="links">';
							$output .= '<a href="'.$image.'" class="icon zoom mustang-gallery"><i class="fa fa-search"></i></a>';
...
function shortcode_our_features($atts, $content = null) {	
   extract(shortcode_atts(array(
	...
	$output .= '<div class="feature-container '.$classname.'"  style="'.$variables.';">';
...
{	
   extract(shortcode_atts(array(
	...
	$output .='<div class="banner" style="'.$variables.';"><a href="'.$link_url.'" target="_blank"><img class="animated" data-animated="fadeInRight" src="'.$image.'" alt=""/></a></div>';	
	return $output;
}
...
function shortcode_blog_posts_container($atts, $content = null, $code) {
	...
	while ( $blog_array->have_posts() ) : $blog_array->the_post();
		...
		$src = mr_image_resize($image, 355, 195, true, 't', false);
		if( empty ( $src ) || $src == 'image_not_specified' ):
			$src = get_template_directory_uri()."/images/megnor/placeholder.png";
			$src = mr_image_resize($src, 355, 195, true, 't', false);
		endif;
			...
			$output .= '<div class="container-inner">';
				...
				$output .= '<div class="post_author1"><i class="blog-icon fa fa-user"></i>'.$post_author.'&nbsp;&nbsp;</div><div class="post-date"><i class="blog-icon fa fa-calendar"></i><div class="day">'.$post_day.'</div><div class="month">'.$post_month.'</div> <div class="year">'.$post_year. '</div></div>';	
				$output .= '</div>';
			...
			if($comments->total_comments > 0):
          		$output .='<div class="comments-link"><i class="fa fa-comments"> </i>'.$comments->total_comments.' comment </div>';
       		else:
          		$output .='<div class="comments-link"> <i class="fa fa-comments"> </i> Leave a comment </div>';
         	endif;
			$output .= '</div></div></div>';
		$i++;
	endwhile;
...
function shortcode_map_address($atts, $content = null) {	
	extract(shortcode_atts(array(
	  'latitude' => '21.182626',
	  'longitude' => '72.831674',
	  'map_height' => '300',
	...
	$output = '';		
	$output .= '<div class="contactmap-wrapper">';
		if(!empty($latitude) && !empty($longitude)) :
			$output .= '<iframe src="https://maps.google.com/maps/ms?msa=0&amp;msid=217335106549557371118.0004d6b26f3bd9a41e129&amp;ie=UTF8&amp;ll='.$latitude.','.$longitude.'&amp;spn=0,0&amp;t=m&amp;output=embed" width="'.$map_width.'" height="'.$map_height.'" frameborder="0" marginwidth="0" marginheight="0" scrolling="no"></iframe>';
			// $output .= '<div class="iframe-image" style="width:'.$map_width.'; height:'.$map_width.'"></div>';
		 endif; 		
	return $output;
}
add_shortcode("tm_map", "shortcode_map_address");
...
add_shortcode('tm_tab', 'shortcode_product_tab');*/
...
$output .= "<{$divtag} class='wp-caption-text gallery-zoom'>
		<a href=" . $image_url . " title='Click to view Full Image' class='icon zoom mustang-gallery'><i class='fa fa-search'></i></a>
	</{$divtag}>";
