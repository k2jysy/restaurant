...
class AboutusWidget extends WP_Widget
{
    function AboutusWidget(){
		...
		parent::WP_Widget(false,$name='TM - About Us Widget',$widget_settings);
    }
    function widget($args, $instance){
		...
		<div class="aboutus_imagecontent animated" data-animated="fadeInLeft" style="<?php echo $imageDiv; ?>;opacity:0">
			<a href="<?php echo esc_url($linkURL);?>" title="<?php echo esc_attr($title);?>" <?php if($window_target == true) echo 'target="_blank"'; ?>> <img src="<?php echo esc_url($imageSrc); ?>" id="<?php echo esc_attr($title);?>" alt="" title="<?php echo esc_attr($title);?>" /> </a>
		</div>
		
		<div class="tm_aboutus_content" style="<?php echo $contentDiv;?>">
			 <?php 
				if($title) : ?>
				<h1 class="aboutus_title"><?php echo esc_attr($title); ?></h1>
				<?php endif; 
			 ?>
			...
			<div class="aboutus_readmore">
				<a href="<?php echo esc_url($linkURL); ?>" title="<?php echo esc_attr($link_text); ?>"><?php echo esc_attr($link_text); ?></a>
			</div>			
		</div>
    ...
    function form($instance){
		...
		<p><label for="<?php echo $this->get_field_id('description');?>"><?php _e('Description:', 'templatemela'); ?></label>
			<textarea cols="18" rows="3" class="widefat" id="<?php echo $this->get_field_id('description');?>" name="<?php echo $this->get_field_name('description');?>" ><?php echo esc_attr($description);?></textarea>
		</p>
		...
		<input class="widefat" id="<?php echo $this->get_field_id('imageSrc');?>" name="<?php echo $this->get_field_name('imageSrc');?>" type="text" value="<?php echo esc_attr($imageSrc);?>" />
		<br />
